...
- `uv run ruff check src tests examples` is a useful signal but the repo may contain pre-existing lint findings.
...
2. **Lint:** `uv run ruff check src tests examples` — no new findings from your diff.
