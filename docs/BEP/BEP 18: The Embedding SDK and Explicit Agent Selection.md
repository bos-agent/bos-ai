...
- the ports: `LLM`, `ChatStore`, `Consolidator`, `ToolSet`, `TurnInterceptor`, `PromptProvider`, `TurnEventSink`, `AgentPort`
- the types those ports' own methods pass and return: `LLMResponse`, `ChatCommit`, `ChatMeta`, `ContextResult`, `TokenEstimate`, `ToolAttributes`, `TurnEvent`, `TextPart`, `ImagePart`, `FilePart`
- `ToolCallRequest` and `MessageContent` — neither is a port-method signature type the drift test below reaches on its own, so both are promised by hand. `ToolCallRequest` is a field of `LLMResponse` (`LLMResponse.tool_calls`), so no port can hand back an `LLMResponse` without it. `MessageContent` is the bare `TypeAlias` `AgentPort.ask`/`run` (BEP 19 §3.3) annotate their `content` parameter as; `typing.get_type_hints()` inlines a plain `TypeAlias` at the call site, so the drift test sees only the `TextPart`/`ImagePart`/`FilePart` it expands to — those three land in the bullet above the same mechanical way `ChatCommit` or `TokenEstimate` did, while `MessageContent` itself is promised alongside them so an implementer building a non-string `content` value has the union's own name, not just its leaves
