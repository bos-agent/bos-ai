...
- **Status:** **Implemented** — Layers 1–3 shipped (#100, #101, #102). Step 13 (documentation) was deferred by design and is discharged by BEP 18 §6 step 8; see §9.
...
- 2026-09-23 — Closed. Layer 3 shipped in #102: `POST /api/restart`, `is_live()`, and the CLI learning to see an embedded gateway rather than printing destructive advice at one. Step 13 stays discharged rather than done-here: it asked for an *Embedding the gateway* page, and holding it until BEP 18 existed turned it into one page covering both shapes (`docs/site/embedding/index.md`) instead of a gateway-only page that would have been rewritten weeks later. One thing this BEP built is now a documented hazard rather than a finished story: `GatewayMount` brings up a runtime without consulting `bos.sdk`'s process guard, so a mounted gateway and a live `BosApp` silently rebuild each other's registry. It is recorded in `docs/BACKLOG.md` §4, because the fix crosses a BEP 13 ring boundary and is a design question, not a patch.
