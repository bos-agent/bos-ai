...
- Commands live in two modules (see 2026-06-13 revision): `src/bos/cli/commands/scaffolding.py` exposes the top-level `init` command and the `gen` generator group, and `src/bos/cli/commands/doctor.py` exposes `doctor`. Each is registered in `_LAZY_COMMANDS` under its own top-level name (`init`, `gen`, `doctor`); `doctor.py` imports the shared `_discover_project`/`_probe_model` helpers from `scaffolding.py`.
...
| 2026-06-13 | `project` group flattened to top level | The `project` group is removed: `project init` → `boscli init`, `project doctor` → `boscli doctor`, and the `project add` generator group is renamed and promoted to `boscli gen` (`gen agent`/`gen tool`/`gen channel`). `add` was misleading (it scaffolds component stubs, not dependencies); `gen` matches the `rails/ng/nest generate` convention. The `gateway` group is intentionally kept (named subsystem; generic verbs like `status`/`start` would be ambiguous at the top level). Hard break, no aliases — consistent with BEP 6's no-shim stance |
