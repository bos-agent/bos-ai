...
of registries, globals, plugins, and config. Turn-setup steps (`_build_system_prompt`,
`_load_and_compact_history`) and the injected ports (`_chat_store`, …) are **private** — `ask()` is the
agent's only public action. (An earlier dev-only `bos debug` CLI reached into these steps for
introspection; it was removed rather than widen the agent's surface — previewing a turn is the agent's own
concern, so leaking its internal steps as public API was the wrong scope.)
...
| 2026-06-23 | **Agent surface kept minimal — `ask()` is the only public action (§1.4).** Reverted the earlier promotion of `Agent.build_system_prompt` to public: `_build_system_prompt`, `_load_and_compact_history`, and `_chat_store` are all private internal turn-setup steps. The only consumer that reached them was the dev-only `bos debug` CLI (system-prompt / messages introspection); rather than widen the agent's surface (or invent a `prepare_turn`), that command group was **removed** (`cli/commands/debug.py`, its `BOS_DEV` registration, `test_debug_prompt.py`). Previewing a turn is an agent-scope concern, so leaking its internal steps as API was the wrong scope. |
