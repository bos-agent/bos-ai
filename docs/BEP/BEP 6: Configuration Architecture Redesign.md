...
**Identity is not inherited.** `agent_name` is dropped from the parent's resolved spec before it is merged under the child. It keys an agent's memory store and is the name the agent speaks under, so inheriting it made a child — and, since chains are transitive, every descendant down to the next agent that set its own — share its parent's store and name. Each agent's `agent_name` is what it sets itself, or else one from `[agent.defaults]` — which stays the global floor, `_parent` or not — or else its own name; a child that wants to share an identity sets `agent_name` explicitly. The same rule applies to a `_parent` given through `agent_cfg` (BEP 19 §3.4.1.1), with one difference: that route reads the parent from the registry, which cannot tell a defaults-layer `agent_name` from one the parent set, so it drops both. A global `agent_name` gives every agent one store and one name, so the difference only shows in a config that is already misconfigured.
...
| 2026-09-25 | `_parent`: identity is not inherited | `agent_name` no longer passes from parent to child, on either the config or the `agent_cfg` route; before, a parent's `agent_name` gave the whole subtree below it one shared memory store and one name. A config whose parent set `agent_name` sees its children start their own memory, under their own names unless `[agent.defaults]` names them; the shared store stays where it was, under the parent's name |
