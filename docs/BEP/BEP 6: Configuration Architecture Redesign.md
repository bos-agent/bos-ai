...
| 2026-09-25 | `_parent`: identity is not inherited | `agent_name` no longer passes from parent to child, on either the config or the `agent_cfg` route; before, a parent's `agent_name` gave the whole subtree below it one shared memory store and one name. A config whose parent set `agent_name` sees its children start their own memory, under their own names; the shared store stays where it was, under the parent's name |
