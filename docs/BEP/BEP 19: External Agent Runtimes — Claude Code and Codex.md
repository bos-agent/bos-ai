...
The runtime declares which settings it sets for itself, and `parse_external_config` **rejects** a `native_options` naming any of them, at construction, with the key in the message. For Codex that set is `sandbox`, `approval_mode`, `cwd`, `model`, `developer_instructions`, `base_instructions`, and `config.mcp_servers` — the whole of what `_thread_for` sends, with `config` as the one exception that is merged rather than owned, so only the sub-key BOS writes there is reserved. Rejected rather than last-write-wins or warned past, because `sandbox` and `approval_mode` are derived from `permission` and `cwd` is the containment boundary: a config that could quietly replace any of them would undo §3.5 while looking like a tuning knob. An escape hatch is for the settings BOS does not touch, and the parse error is what keeps that true.

**What the reserved set does and does not buy.** It stops `native_options` *renaming* a decision BOS sends: for the settings BOS passes as typed parameters, measurement confirms the parameter wins over anything the `config` layer says — checked for `sandbox_mode`, `approval_policy`, `cwd` and `model`, which is every one that decides what the runtime may do. What it cannot do is make the `config` table safe in general, because Codex lets a decision be reached under a second name: `sandbox_mode` in `config` loses to BOS's `sandbox`, but `sandbox_workspace_write.writable_roots` is not a typed parameter at all, and under `permission = "workspace-write"` the server honours it. That table is therefore reserved too, wholesale. **The list is a denylist arrived at by probing, not an enumeration** — there is no listing of Codex's config surface that BOS can read, so it is not provably complete, and §8.2 carries what that leaves open. Read `native_options` as config trusted at the same level as `permission`, written by the same person, and not as something the runtime bounds.

A name the runtime's call does not have is *not* caught here — BOS does not carry a copy of the vendor's signature — so it surfaces as a `TypeError` from the vendor on the first turn, which names the key and suggests the near miss. That is the intended trade: an escape hatch that silently dropped what it did not recognise would be worse than one that fails loudly. A *table* BOS reserves inside (`config`) is checked, though, because a non-table there would fail inside BOS's own merge and name nothing.
...
- **`native_options`' reserved list is a denylist, and nothing proves it complete.** §3.4 rejects a `native_options` naming any setting the runtime sends as a typed parameter, plus `config.mcp_servers` and `config.sandbox_workspace_write`. The first group is provably closed: it is derived from `_BOS_THREAD_KWARGS`, which a test pins to the keywords actually sent. The second is not. `sandbox_workspace_write` was found by probing candidates against the `SandboxPolicy` the server resolves — seven candidate settings probed, one moved it — and probing is not enumerating. There is no listing of Codex's config surface BOS can read: no `codex mcp get --json` equivalent for top-level config, and the app-server schema models `ThreadStartParams.config` as an untyped object. Two axes the probe could not see at all, both real: settings that change what the child's *shell* inherits (`shell_environment_policy`) rather than what the sandbox permits — which also decides whether the agent can read BOS's own MCP bearer token out of its environment (§3.8) — and settings that grant reach through a *tool* rather than the sandbox. Neither is reserved, because neither was shown to move anything and reserving on a guess blocks a legitimate knob. **The stance this rests on:** `native_options` is trusted config, written by the same person who writes `permission`, so an incomplete denylist is a missing guard-rail and not an escalation path — nothing an agent can reach. Revisit if that stops being true (a host that takes `native_options` from somewhere less trusted than its own config), or if the vendor ever publishes a config schema, at which point the denylist can become an allowlist derived from it.
