...
- **Status:** **Partially implemented** — §6 Layers 1–3 shipped, Layer 4's **Codex** runtime built and Layer 5's `codex` extra packaged; the Codex runtime validated live against a real ChatGPT subscription (three runs on 2026-09-25, §8.1); Layer 4's Claude Code runtime and its live validation not started. See §8.1 for readiness per track and §9 for what changed.
...
| Project docs under `cwd` | The repo | `CLAUDE.md`, via `setting_sources` (§3.5.3) | `AGENTS.md`, read by codex; no knob observed to suppress it (§3.4.1.4) |
...
| `workspace-write` | `Sandbox.workspace_write` + `ApprovalMode.deny_all` | `permission_mode="acceptEdits"`, `can_use_tool` path check, `sandbox={"enabled": True}` |
| `full-access` | `Sandbox.full_access` + `ApprovalMode.deny_all` | `permission_mode="bypassPermissions"`, no path check |

**`workspace-write` can write `/tmp`, by Codex's default, and BOS documents that rather than overriding it.** The resolved policy has `excludeSlashTmp: false` and `excludeTmpdirEnvVar: false`. Observed in live runs 2 and 3: a `workspace-write` agent wrote a file under `/tmp` outside its `cwd`. `$TMPDIR` was unset in both runs, so a write there was not observed; the policy says it is writable too. A host that needs `/tmp` excluded has no BOS-level setting for it — `config.sandbox_workspace_write` is reserved (§3.4).
...
| `request_stop()` | same, raced against the native turn; a turn *started* after it returns the `SHUTDOWN_CONTENT` marker before any vendor call, as `Agent` does | same |
| `event_sink` | `receive_response()`: `ToolUseBlock`→`tool`/`start`, `ToolResultBlock`→`tool`/`finish`, `TextBlock`→`response`, `ResultMessage`→`turn`/`finish` | `AsyncTurnHandle.stream()`: `item/started`·`item/completed` on a command or MCP-tool item→`tool`, `item/completed` on an agent message→`response`/`finish`, `turn/completed`→`turn`/`finish`; every other item kind is skipped rather than half-mapped |
...
`AgentResult` is populated from `ResultMessage` (`usage`, `total_cost_usd`, `num_turns`, `terminal_reason`) or `TurnResult` (`usage` → `usage`, `status` → `finish_reason`). `finish_reason` carries the native terminal reason verbatim. `TurnResult.error` and `.duration_ms` are **dropped** — `AgentResult` has no field for either — and the first costs nothing, because a failed turn never reaches this path: the vendor raises before constructing a `TurnResult`, and BOS re-raises with that message.
...
- A missing extra, an escaping `cwd`, an unknown config key, a `native_options` naming a setting the runtime sets itself, and `workspace-write` on Claude Code where the bash sandbox is unavailable all fail at `create_agent` with a message naming the fix. **An absent login does not** — the `auth` preflight is an RPC and runs on the first turn (§3.10.3), so `create_agent` succeeds and the first `ask()` raises.
