...
| `BosToolMcpServer` | An in-process `uvicorn.Server` on `127.0.0.1:<ephemeral>` serving one Starlette app | `bos/extensions/runtimes/mcp_egress.py` | One per harness, started lazily the first time an external agent declares at least one `mcp_tools` name the host has an `ep_tool` for (§3.8) — a list that is empty, or wholly unmatched, starts none | Closed by `_aclose` last, after every runtime (§3.8) |
...
**`native_options` is the escape hatch, and it cannot reach what BOS decides.** Its entries are handed to the runtime's own session-start call as keywords — for Codex, `thread_start`/`thread_resume`, so `personality = "concise"` above becomes `thread_start(personality="concise")`. One entry is treated differently and says so: a `config` sub-table is *merged into* the `config=` override rather than passed as a keyword, because Codex's own `config.toml` keys are not keywords — that is the route §3.4.1.4's `project_doc_max_bytes` takes.
...
A name the runtime's call does not have is *not* caught here — BOS does not carry a copy of the vendor's signature — so it surfaces as a `TypeError` from the vendor on the first turn. That is the intended trade: an escape hatch that silently dropped what it did not recognise would be worse than one that fails loudly.
