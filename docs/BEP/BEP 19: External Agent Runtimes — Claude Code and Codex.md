...
20. `timeout_seconds` expiry raises. Precondition: it bounds one turn *attempt*, not the whole call — each schema retry gets a fresh window. Expiry while the turn is streaming interrupts the native turn first; expiry during thread setup has no turn to interrupt and says so in the message.
