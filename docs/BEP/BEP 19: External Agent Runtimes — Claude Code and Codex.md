...
- **Status:** **Partially implemented** — §6 Layers 1–3 shipped, Layer 4's **Codex** runtime built and Layer 5's `codex` extra packaged; Layer 4's Claude Code runtime and Layer 5's live validation not started. See §8.1 for readiness per track and §9 for what changed.
...
25. With `system_prompt` set, the runtime's reply reflects it while its own tool guidance still works — the pairing §3.4.1 protects. With `native_options = { config = { project_doc_max_bytes = 0 } }`, Codex's reply no longer reflects an `AGENTS.md` placed in `cwd`. Two preconditions, and the first is now met in CI: the key has to *arrive*, which is `thread_start(config=…)` and is asserted by `test_native_options_config_merges_rather_than_replacing`; and it has to *work*, which only a live turn shows. If the reply still reflects `AGENTS.md`, that falsifies §3.4.1.4's sentence about the knob rather than showing BOS dropped it — a distinction this criterion could not draw before the route existed.
