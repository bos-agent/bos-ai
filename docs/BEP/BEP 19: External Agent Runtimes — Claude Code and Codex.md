...
`.get`, not `.pop`: `external_runtime` **stays in `merged_cfg`**, which becomes the runtime's own `cfg`, because `boscli inspect` reads `cfg.get("external_runtime")` off the built agent to report which vendor backs it (§3.3.1). Popping it would answer "what is `george`'s `_parent`?" correctly once, at construction, and then erase the only record of the answer.
...
For Codex the endpoint is injected per thread, not per process: `thread_start`/`thread_resume` take a `config` object that is merged over `~/.codex/config.toml`, and BOS passes `{"mcp_servers": {"bos": {"url": server.url, "http_headers": {"Authorization": "Bearer <token>"}}}}`. The server name is one constant (`_MCP_SERVER_NAME`) because it is shared with the operator's own config — see §8.2. `http_headers` and not `bearer_token` (which fails config loading outright) and not `bearer_token_env_var` (which names an environment variable, and `CodexConfig(env=…)` is fixed before the token exists). The whole of it is built once, inside `_ensure_client`, so an agent that never runs a turn never asks the harness for a server — and an agent whose `mcp_tools` is empty never asks either, which is what keeps the port unbound rather than merely unused.
...
5. `mcp_tools = ["NoSuchTool"]` logs exactly one warning naming the agent and the tool, and grants that agent nothing. It does *start* a server, though: `mcp_tools = []` — the default — is the case that starts none and never even asks the harness for one, and the two are not the same case. A non-empty list has to reach `register_agent`, because `register_agent` is what emits the warning, and an operator who mistyped their only tool name needs that line more than they need the port not bound.
...
| Layer 4 Codex | **Built**, this branch — `CodexAgent` and its MCP egress wiring. §7.15–26 remain **unverified** until run against a real ChatGPT login, and §8.2 names the one wiring claim inside them that no fake can reach (the `config=` channel). The vendor-module-classification gap and the empty-`mcp_tools` half of §7.7 are closed; the child-process half of §7.7 and §7.27 is not, and cannot be until a real `codex app-server` is spawned — which is the same live run. |
...
- **The Codex MCP wiring is verified only up to the config dict.** §3.8's Codex half is `thread_start`/`thread_resume`'s `config={"mcp_servers": {"bos": {"url": …, "http_headers": {"Authorization": "Bearer …"}}}}`, and CI can assert that dict and that BOS's own server answers the header in it — `tests/test_codex_mcp_wiring.py` does both, the second against a real `BosToolMcpServer` over HTTP. What CI cannot reach is the child: that `codex app-server` reads that override at all, and then lists and calls the BOS tool through it (§7.22). The key choices behind the dict were measured against codex-cli 0.145.0 with a throwaway `CODEX_HOME`, but through the CLI's own `-c` flag, not through `config=`: `http_headers` is recognized (`codex mcp list` shows `Auth: Bearer token`, where an invented key shows `Unsupported`), `bearer_token` is not a literal-token key and fails the whole config load ("bearer_token is not supported for streamable_http"), and `bearer_token_env_var` names an environment variable BOS cannot fill, since `CodexConfig(env=…)` is fixed before the token is minted. That the two are one channel is inference from the parameter's name and position; the app-server schema documents `ThreadStartParams.config` as nothing but a free-form object. Task 11 item 8 is where it is settled. If it turns out not to be that channel, the fallback is the same TOML through `CodexConfig(config_overrides=…)`, which the SDK expands into `--config <kv>` on the child's own command line (`client.py:253-254`) — the exact flag the measurements above used. It is the fallback and not the first choice because a command line is world-readable to any local `ps`, and the bearer token would be on it.
- **BOS's `[mcp_servers.bos]` entry shares a namespace with the operator's own.** A consequence of the config-isolation entry below, but a sharper one, because `config=` *merges* rather than replaces. Measured on codex-cli 0.145.0: the merge is per key even for a whole-table override — overriding `mcp_servers.bos` with a complete `{url, http_headers}` table left a `bearer_token_env_var` from `config.toml` in place — and a collision with a *stdio* entry of the same name fails the whole config load ("url is not supported for stdio"), taking the turn with it. Not resolved, because every fix is worse: a random or hashed name is unreadable in the operator's own config, and `CODEX_HOME` cannot be relocated for the reason below. The failure that matters is loud, and the name is one constant (`_MCP_SERVER_NAME`).
