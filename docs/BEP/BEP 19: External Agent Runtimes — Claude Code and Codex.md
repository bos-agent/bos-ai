...
- **The Codex MCP wiring is verified only up to the config dict.** §3.8's Codex half is `thread_start`/`thread_resume`'s `config={"mcp_servers": {"bos-tools": {"url": …, "http_headers": {"Authorization": "Bearer …"}}}}`, and CI can assert that dict and that BOS's own server answers the header in it — `tests/test_codex_mcp_wiring.py` does both, the second against a real `BosToolMcpServer` over HTTP. What CI cannot reach is the child: that `codex app-server` reads that override at all, and then lists and calls the BOS tool through it (§7.22). The key choices behind the dict were measured against codex-cli 0.145.0 with a throwaway `CODEX_HOME`, but through the CLI's own `-c` flag, not through `config=`: `http_headers` is recognized (`codex mcp list` shows `Auth: Bearer token`, where an invented key shows `Unsupported`), `bearer_token` is not a literal-token key and fails the whole config load ("bearer_token is not supported for streamable_http"), and `bearer_token_env_var` names an environment variable BOS cannot fill, since `CodexConfig(env=…)` is fixed before the token is minted. That the two are one channel is inference from the parameter's name and position; the app-server schema documents `ThreadStartParams.config` as nothing but a free-form object. Task 11 item 8 is where it is settled. If it turns out not to be that channel, the fallback is the same TOML through `CodexConfig(config_overrides=…)`, which the SDK expands into `--config <kv>` on the child's own command line (`client.py:253-254`) — the exact flag the measurements above used. It is the fallback and not the first choice because a command line is world-readable to any local `ps`, and the bearer token would be on it.
- **BOS's `[mcp_servers.bos-tools]` entry shares a namespace with the operator's own, and a collision is not cosmetic.** A sharper case of the config-isolation entry below, because `config=` *merges* rather than replaces. What an operator would see, if they already have a table of that name in `~/.codex/config.toml`: with a **stdio** entry, every turn fails outright, because the whole config stops loading ("url is not supported for stdio in `mcp_servers.bos-tools`") — loud, and at least it names the table. With an **HTTP** entry it is the quiet one: the merge is per key even for a whole-table override — measured on codex-cli 0.145.0, overriding the table wholesale with a complete `{url, http_headers}` still left a `bearer_token_env_var` from `config.toml` in place — so Codex holds two credentials for one server and BOS cannot say which `Authorization` it sends. If it sends theirs, `_gate` answers 401, the agent simply has no BOS tools, and nothing says so: `_gate` does not log a refusal and the server runs with `access_log=False`. Mitigated, not resolved, by the name: `_MCP_SERVER_NAME` is `bos-tools`, which is what the server already reports for itself over MCP and is a good deal less likely to be a table someone already wrote than `bos`. Not resolved further because every remaining fix is worse — a random or hashed name is unreadable in the operator's own config, and `CODEX_HOME` cannot be relocated, for the reason below. Revisit by logging the refusal in `_gate`, which would at least make the quiet case visible; deferred because an unauthenticated endpoint that logs on every probe is its own problem.
