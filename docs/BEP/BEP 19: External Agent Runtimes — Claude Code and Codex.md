...
- 2026-09-25 — **`_parent` in `agent_cfg` is resolved for the reserved runtimes, and refused otherwise.** Found while answering where an embedder sets a Codex agent's `permission`: `build_agent("martha", agent_cfg={"_parent": "codex", "permission": "read-only"})` built a plain BOS `Agent`, because `agent_cfg` bypasses the workspace resolver and `_apply` silently filtered `_parent` and `permission` out of `Agent`'s kwargs — the caller asked for a sandboxed runtime and got an agent with BOS's own tools and no error. `create_agent` now resolves a reserved `_parent` exactly as `_EXTERNAL_RUNTIME_SPECS` does, and refuses any other parent, a re-parenting of a kind with its own config, and a `_parent` that contradicts the runtime the kind or an `external_runtime` names (§3.4.1.1).
