...
`.get`, not `.pop`: `external_runtime` **stays in `merged_cfg`**, which becomes the runtime's own `cfg`. Popping it would answer "what is `george`'s `_parent`?" correctly once, here, and then erase the answer before anything else can read it — `parse_external_config` lists the key among the ones a runtime honours (`_shared.py`'s `_KNOWN_KEYS`), and the runtime restates it in `resolved_config`, which is where `boscli inspect` reads the backing vendor from (§3.3.1).
...
| Layer 4 Codex | **Built**, this branch — `CodexAgent` and its MCP egress wiring. §7.15–26 remain **unverified** until run against a real ChatGPT login, with one exception: the first two thirds of §7.22 — the runtime *lists* the exposed BOS tool over MCP — is proven in CI against the real `codex app-server`, which needs no login to start a thread. Only the model *calling* the tool still waits on one. The vendor-module-classification gap and the empty-`mcp_tools` half of §7.7 are closed; the child-process half of §7.7 and §7.27 is not, and cannot be until a real `codex app-server` is spawned — which is the same live run. |
