...
A `_parent` in `agent_cfg` names a reserved runtime or any registered agent, and resolves to what the same settings under `[agents.<name>]` with that `_parent` would: a registered agent's `AgentRegistry` defaults already hold its whole resolved chain, so they are deep-merged under `agent_cfg`, with the child's own name as its `kind` — per-agent plugin state such as memory is keyed by `agent_name or kind`, and the parent's `agent_name` is not inherited, on this route as in config (BEP 6's `_parent` addendum), so each child gets a store of its own unless it names one — and a reserved runtime is seeded with the marker `_EXTERNAL_RUNTIME_SPECS` writes, its own `[agents.<runtime>]` config included when there is one. The settings are given in `agent_cfg`'s own shape, which is the harness's argument shape rather than TOML's (`tools` is a list, not a `[tools]` table). So `build_agent("martha", agent_cfg={"_parent": "codex", "permission": "read-only"})` is a second Codex instance, and `build_agent("george-b", agent_cfg={"_parent": "george", "cwd": "services/b"})` is `george` in another directory. `create_agent` refuses, rather than drops, a `_parent` that is neither; one given to a kind that is already registered, whose defaults fold in a lineage of their own; and one whose runtime contradicts the kind or an `external_runtime` in the same `agent_cfg` ([`harness.py`](../../src/bos/core/harness.py)'s `_resolve_agent_cfg_parent`). An actor's `agent_cfg` is the exception: an actor selects a declared agent with `agent`, so `_parent` there is refused when the config loads, naming the actor. The TOML equivalent for a per-actor profile is a section — which, unlike an inline table, can hold a multi-line string:
