...
7. ✅ **Collapse `ToolContext` onto `ParentTurn`.** Done. `ToolContext` now *composes* a single `parent: ParentTurn` (plus `extra_data`) instead of re-declaring the four turn fields; the derived `.parent` property is gone. `ParentTurn.agent_name` is now required (`str`), matching the old `ToolContext.agent_name`. Turn fields are read via `ctx.parent.*` — the blast radius was small: the only real `ToolContext` field-read site outside the constructor was `skills/plugin.py` (`context.parent.agent_name`); the many `context.chat_id`/`turn_id` reads in `task.py`/`plan.py` are on `TurnContext` (interceptor/prompt-section params), not `ToolContext`, and were correctly left untouched.
...
- **2026-06-26** — Open Issue #7 resolved: `ToolContext` now composes a single `parent: ParentTurn` (the four turn fields removed, `.parent` property dropped, `ParentTurn.agent_name` made required). Turn fields read via `ctx.parent.*`.
