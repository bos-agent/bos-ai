...
Tools may still run before the final answer — so an agent can *research, then propose a typed result* (the capability the former "Agent-Backed Command Workflow" BEP §A4 assumed; that BEP was withdrawn 2026-07-03 and its number 15 reused).
...
| Consumer | former "Agent-Backed Command Workflow" BEP §A4 (withdrawn 2026-07-03) | gained the structured-output primitive it presumed | n/a |
...
7. ✅ **Collapse `ToolContext` onto `ParentTurn`.** Done. `ToolContext` now *composes* a single `parent: ParentTurn` (plus `extra_data`) instead of re-declaring the four turn fields; the derived `.parent` property is gone. `ParentTurn.agent_name` is now required (`str`), matching the old `ToolContext.agent_name`. Turn fields are read via `ctx.parent.*`. The many `context.chat_id`/`turn_id` reads in `task.py`/`plan.py` are on `TurnContext` (interceptor/prompt-section params), not `ToolContext`, and were correctly left untouched.

   Also collapsed the **tool-invocation surface**: the agent used to inject flat `chat_id`/`turn_id`/`event_sink` kwargs into every tool call *alongside* `context`. `turn_id`/`event_sink` were dead (no tool read them); `chat_id` was read by the task/plan tools. Those eight tools (`Task{Create,Update,List,Get}`, `Plan{Create,Update,Get,Clear}`) now take `context: ToolContext` and read `context.parent.chat_id`, and `Agent._invoke_tool` folds the runtime linkage into a single `ToolContext` — tools no longer receive any flat turn parameter. `context` is the one runtime injection.
...
- Former BEP 15: Agent-Backed Command Workflow (§A4 structured-output contract — was the primary consumer; withdrawn 2026-07-03, number reused by "BEP 15: Built-in Config Agent for BOS Project Configuration").
...
- **2026-07-03** — "BEP 15: Agent-Backed Command Workflow" withdrawn and deleted; number 15 reused by "Built-in Config Agent for BOS Project Configuration". References here updated to say "former BEP 15"; no design content in this BEP changes.
