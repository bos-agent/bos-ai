...
- `agent.py`
  `ReactAgent`, `TurnContext`, `AbortTurn`, and the interceptor chain used by the agent loop.
- `actor.py`
...
`FileSystemSkillsLoader` discovers `SKILL.md` files from configured skill
directories and exposes typed skill metadata (`location`, `summary`) through
`search_skills()`. `load_skill()` returns the full skill file text directly so
agents can consume skill instructions as tool results without maintaining a
separate active-skill mutation layer.
