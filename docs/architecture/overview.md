...
- `bos.runner`
  In-process composition of harness, actor, and channels.
...
1. `Workspace` resolves configuration from an ancestor `.bos/` or explicit
   `BOS_DIR`, then loads `config.toml`.
2. `workspace.bootstrap_platform()` resolves runtime-facing platform config,
   then loads extension modules and registers configured agents.
