...
async def get_antigravity_token(auth_file: str | None = None) -> OAuthCredentials:
    ...
    if not token_path.exists():
        raise RuntimeError(
            f"Antigravity credentials not found at {token_path}. Run `boscli auth antigravity` to authenticate first."
        )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Run `boscli auth antigravity` to re-authenticate. Error: {e}"
        )
    ...
    for msg in messages:
        ...
        if role == "system":
            ...
            if isinstance(content_val, list):
                raise ValueError(
                    "Structured multimodal inputs are not supported by the antigravity provider in phase 1."
                )
...
async def antigravity_complete(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        return LLMResponse(content=f"Error calling Antigravity: {exc}", finish_reason="error")
