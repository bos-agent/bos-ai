...
def test_initialize_workspace_rejects_already_initialized(tmp_path, monkeypatch):
    workspace = tmp_path / "project"
    (workspace / ".bos").mkdir(parents=True)
    (workspace / ".bos" / "config.toml").write_text("", encoding="utf-8")
    monkeypatch.delenv("BOS_CONFIG", raising=False)
...
# --- .bos/config.toml discovery ---


def test_workspace_loads_from_dotbos_config(tmp_path, monkeypatch):
    monkeypatch.delenv("BOS_CONFIG", raising=False)
    workspace = tmp_path / "project"
    (workspace / ".bos").mkdir(parents=True)
    (workspace / ".bos" / "config.toml").write_text('[platform]\nextensions = ["bos.exts"]\n', encoding="utf-8")

    ws = Workspace.from_discovery(workspace)

    assert ws.bos_dir == (workspace / ".bos").resolve()
    assert ws.config.platform is not None
    assert ws.config.platform.extensions == ["bos.exts"]


def test_workspace_dotbos_discovered_from_subdir(tmp_path, monkeypatch):
    monkeypatch.delenv("BOS_CONFIG", raising=False)
    workspace = tmp_path / "project"
    ...
    assert ws.bos_dir == (workspace / ".bos").resolve()
    assert ws.workspace == workspace.resolve()


def test_workspace_ignores_root_bos_toml(tmp_path, monkeypatch):
    """A legacy root ``bos.toml`` is no longer a config source."""
    monkeypatch.delenv("BOS_CONFIG", raising=False)
    workspace = tmp_path / "project"
    workspace.mkdir()
