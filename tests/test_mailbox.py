...
class FakeAgent:
    ...
    async def ask(self, chat_id: str, message: str) -> str:
...
class TestJsonlMailRoute:
    ...
    @pytest.mark.asyncio
    async def test_bound_send_round_trips_structured_message_content(self, tmp_path):
        route = JsonlMailRoute(store_dir=tmp_path)
        sender = route.bind("alice")
        receiver = route.bind("bob")
        await receiver.receive_nowait()
        ...
        await sender.send("bob", content, chat_id="thread-structured")
        result = await receiver.receive_nowait()

        assert result is not None
        assert result.content == content
        assert result.chat_id == "thread-structured"

    @pytest.mark.asyncio
    async def test_bound_send_rejects_structured_non_message_content(self, tmp_path):
        route = JsonlMailRoute(store_dir=tmp_path)
        sender = route.bind("alice")
